...
export default class kuwo {
  ...
  static search(url) {
    ...
    const target_url = `https://www.kuwo.cn/api/www/search/${api}?key=${encodeURIComponent(keyword)}&pn=${curpage}&rn=20`;
    return new Promise((resolve) => {
      ipcRenderer.invoke("search-kuwo", target_url).then(res => {
        let data = res.data;
        if (!data) {
          resolve({
            lists: [],
        ...
        resolve({
          lists: data.list.map(item => {
            return this.kw_convert_song2(item)
          }),
          total: +data.total,
          platform: 'kuwo'
        })
  ...
  static bootstrap_track(track) {
    const sound = {};
    const song_id = track.id.slice('kwtrack_'.length);
    const target_url =
      'https://antiserver.kuwo.cn/anti.s?' +
      `type=convert_url&format=mp3&response=url&rid=${song_id}&callback=url`;

    return axios.get(target_url).then((data) => {
      if (data.length > 0) {
        sound.url = data;
        sound.platform = 'kuwo';
  ...
  static show_playlist(url) {
    ...
    const target_url = `https://www.kuwo.cn/api/pc/classify/playlist/getRcmPlayList?pn=${offset / 25 + 1
